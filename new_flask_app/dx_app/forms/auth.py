...
from wtforms import (StringField, PasswordField, SubmitField, SelectField,
                     DateField, EmailField, TelField, SelectField, BooleanField, IntegerField, TextAreaField,
                     )
...
class RegisterForm(FlaskForm):
    ...
    user_type = SelectField("アカウントタイプ", choices=[("student", "学生"), ("educator", "教務スタッフ"),("guardian", "保護者")], validators=[DataRequired()])
...
class RegistrationFormForEducator(FlaskForm):
    organization_id = StringField("所属ID", validators=[DataRequired(), Length(max=14)])
    depertment_id = StringField("教室ID", validators=[DataRequired(), Length(max=14)])
    hire_date=DateField("入社日", validators=[DataRequired()])
    permissions = SelectField("権限(管理者より伝えられたものを選択してください。)", choices=[(1,"権限レベル1"),(2,"権限レベル2"),(3,"権限レベル3"),(4,"権限レベル4"),(5,"権限レベル5")], validators=[Optional()])
    submit = SubmitField("次へ")