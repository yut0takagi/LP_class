<h1>Practista 「つまづく」がなくなる学習アプリ</h1>


<h1>現在作成中です。</h1>

![fig](https://github.com/yut0takagi/LP_class/blob/main/fig/%E3%83%9B%E3%83%BC%E3%83%A0%E3%83%9A%E3%83%BC%E3%82%B8.png)
...
* 生徒と教務スタッフの情報共有の円滑化
    授業のコマ割り振りを本システムで行うため、生徒の情報も教務スタッフの情報も持つことになります。そのため、割り振りを行ってからそのまま保護者様,生徒本人に伝わるように、保護者様,生徒様向けのページを作成することで、効率的に情報発信が行えると考えました。
* RAGを活用した教育支援システム
    従来の学習システムでは、学習指導に長けているサービスか業務効率化に長けているサービスがほとんどです。具体的には、学習指導に長けているサービスでは、授業配信サービスの導入等により学習環境の提供が行われております。しかし、それらには業務的な面は含まれていません。SaaSサービスにおいて包括的に管理できるサービスはあまり多くはないと感じました。そこで、AIを用いた教務スタッフ側のチャットボット機能を導入するため、生徒側にも導入することで教務スタッフに相談できなかった部分の質問等が行えるため、今までは授業中での理解度把握のみだったものが、宿題実施時にも理解度をはかることができると考えました。具体的なシステム構成としては、RAGを用いることで、生徒が各科目,各分野ごとに課題や授業内容を読み込ませAIに部分的に学習させ、本人にあった学習を提供することができると考えました。

### 2.2.2. 画面遷移図
画面遷移図(教務スタッフ)は以下の通りです。
...
1. ホームページ
    1. ホーム
    2. プロダクト
        Practista以外のプロダクトについて確認できます。
    3. ソリューション
    4. リソース
    5. プラン
        学生,保護者向けのプランを確認できます。
        教育事業者向けの導入プランの場合は規模により費用が上下するため記載しておりません。
    6. ログイン/新規登録
2. ログイン画面
    本ツールでは、学生,教務スタッフ,保護者が利用すると想定するため、以下の情報を取得することとしました。
    ※ 以下の個人情報は、Userテーブル上に作成し、各ロールごとに必要となってくる情報については、別のテーブルで管理することとしています。
    > 1. 氏名(姓と名)
    > 2. メールアドレス
    > 3. パスワード
    > 4. ロール(学生,教務スタッフ,保護者)
    このうち、メールアドレスとパスワードをログイン時必要要件としています。
    <br>
3. 各ロール 必要情報の入力
    先に入力した情報は全ロールで共通に必要となってくるため、各ロールのみの情報はログイン後の入力としました。
    1. 生徒
        生徒の場合、塾生か個人かによって必要となる情報が変わってきます。
        1. 塾生の場合
            1. 
1. シフトの提出と確認
...
2. 生徒管理
    ...
    * 生徒情報編集画面
        ...
        * 授業入れ込み画面
            ...
            従来では、Excelでデータ管理し、それを一度印刷し照らし合わせて評価するのに比べて、プルダウンで選択してくことで容易にコマの振替や代行を行えるようになりました。
